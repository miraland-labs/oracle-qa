...
async fn health(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    let oracle_balance_lamports = rpc.get_balance(&state.config.oracle_pubkey()).await.ok();
    let registry_ok = registry_probe(&state.config).await;
...
async fn registry_probe(config: &OracleConfig) -> bool {
    let Some(base) = config.evidence_registry_urls.first() else {
        return false;
    };
